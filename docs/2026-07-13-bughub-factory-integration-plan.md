...
- [x] 再現欠陥: true headlessを使わず、専用profile・loopback CDP・固定画面外座標でChromeを起動する製品所有launcherを追加する。重複起動を避け、ChatGPT app読込完了まで待ち、README／AI installer契約を手動`open`から正規CLIへ統一した。引数fixtureとmacOS冷起動smokeで固定した。
